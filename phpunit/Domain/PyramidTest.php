...
class PyramidTest extends TestCase{
    ...
    public function test_get_neighbours() {
        print("test_get_neighbours\n");
        // Arrange
        $position = [5, 11];
        $expected_positions = [[7,11], [3,11], [5,13], [5,9]];

        $this->sut = Pyramid::create([]);

        // Act
        $positions =$this->sut->get_neighbours($position);
    ...
    public function test_get_adjacent_positions_first_stage($tiles, $expected_positions) {
        print("test_get_adjacent_positions_first_stage\n");
        // Arrange
        $this->sut = Pyramid::create($tiles);
    ...
    static public function adjacent_positions_provider_first_stage(): array {
        ...
        return [
            ...
            [[$initial0, $faraway_4x4], [ // 4x5 and 5x4 allowed, so not the 8,8 and 18,18 corner
                $t10_8_0, // Dominoes allowed because of 10, 10 tile
                $t12_10_0, $t10_12_0,
                $t8_10_1, 
                $t12_10_1, $t10_12_1,
                $t12_8_2,
                ...
                $t16_14_3, $t18_16_3,
                $t14_16_3,
                // Not allowed to prevent single tile spaces that cannot be filled with dominoes
    ...
    #[\PHPUnit\Framework\Attributes\DataProvider('combine')]
    ...
    static public function combine(): array {
        $box_2_1 = PyramidTest::get_box(8, 8, 14, 12);
        $box_2_3 = PyramidTest::get_box(8, 8, 14, 16);
        $box_2_3_corner = $box_2_3 + PyramidTest::get_box(12, 12, 12, 14);
        return [
            [
                [
                ],
            ...
            [
                ...
                $box_2_1,
                [
                    ['stage' => 1, 'horizontal' => 10, 'vertical' => 10, 'rotation' => 0],
    ...
    #[\PHPUnit\Framework\Attributes\DataProvider('is_candidate_position_free')]
    public function test_is_candidate_position_free($occupied, $position, $rotation, $expected_result) {
        print("test_is_candidate_position_free\n");
        // Arrange
        $this->sut = Pyramid::create($occupied);

        // Act
        $result = $this->sut->is_candidate_position_free($occupied, $position, $rotation);
        // Assert
        // print_r(array_keys($result));
        $this->assertEqualsCanonicalizing($expected_result, $result);
    }
    static public function is_candidate_position_free(): array {
        $box_2_1 = PyramidTest::get_box(8, 8, 14, 12);
        $box_2_3 = PyramidTest::get_box(8, 8, 14, 16);
        $box_2_3_corner = $box_2_3 + PyramidTest::get_box(12, 12, 12, 14);
        return [
    ...
    public function test_get_free_contiguous_area($occupied, $position, $expected_positions) {
        print("test_get_free_contiguous_area\n");
        // Arrange
        $this->sut = Pyramid::create($occupied);

        // Act
        $positions = $this->sut->get_free_contiguous_area($occupied, $position);
        // Assert
        // print_r(array_keys($positions));
        $this->assertEqualsCanonicalizing($expected_positions, $positions);
    }
    static public function get_free_contiguous_area(): array {
        $box_2_1 = PyramidTest::get_box(8, 8, 14, 12);
        $box_1_1 = PyramidTest::get_box(8, 8, 12, 12);
        $box_2_3 = PyramidTest::get_box(8, 8, 14, 16);
        $box_2_3_corner = $box_2_3 + PyramidTest::get_box(12, 12, 12, 14);
        $box_3_3_T = PyramidTest::get_box(8, 8, 16, 16) + PyramidTest::get_box(12, 10, 12, 12);
        ...
        return [
            ...
            [
                $box_2_3_corner,
                [10, 10],
                [[10, 10], [10, 12], [10, 14], [12, 10]],
            ],
            [
                $box_3_3_T,
                [10, 10],
                [[10, 10], [10, 12], [10, 14], [12, 14], [14, 10], [14, 12], [14, 14]],
            ],
        ];
    }
    static public function get_box($horizontal_min, $vertical_min, $horizontal_max, $vertical_max): array {
        $occupied = [];
        for ($h = $horizontal_min; $h <= $horizontal_max; $h += 2) {
            $occupied[Pyramid::calculate_key_horizontal_vertical([$h, $vertical_min])] = Pyramid::calculate_key_horizontal_vertical([$h, $vertical_min]);
            $occupied[Pyramid::calculate_key_horizontal_vertical([$h, $vertical_max])] = Pyramid::calculate_key_horizontal_vertical([$h, $vertical_max]);
        }
        for ($v = $vertical_min; $v <= $vertical_max; $v += 2) {
            $occupied[Pyramid::calculate_key_horizontal_vertical([$horizontal_min, $v])] = Pyramid::calculate_key_horizontal_vertical([$horizontal_min, $v]);
            $occupied[Pyramid::calculate_key_horizontal_vertical([$horizontal_max, $v])] = Pyramid::calculate_key_horizontal_vertical([$horizontal_max, $v]);
        }
        return $occupied;
    }
}
